...
print("Initializing Delta Studio (NPC/Dummy Animation Sync)...")
...
local DummySelectBtn = setupButton("👁 Click Dummy to Record", 0, Color3.fromRGB(100, 70, 150))
local NPCSelectBtn = setupButton("👁 Click NPC to Play On", 0.5, Color3.fromRGB(100, 150, 70))
local TargetStatusLabel = Instance.new("TextLabel")
TargetStatusLabel.Size = UDim2.new(0.9, 0, 0, 25)
TargetStatusLabel.BackgroundColor3 = Color3.fromRGB(45, 45, 45)
TargetStatusLabel.Text = "Targets: None"
TargetStatusLabel.TextColor3 = Color3.fromRGB(200, 200, 200)
TargetStatusLabel.Font = Enum.Font.SourceSans
TargetStatusLabel.TextSize = 12
TargetStatusLabel.BorderSizePixel = 0
TargetStatusLabel.LayoutOrder = 0.7
TargetStatusLabel.Parent = MainFrame

-- Dummy Selection
local dummySelectMode = false
DummySelectBtn.MouseButton1Click:Connect(function()
	dummySelectMode = not dummySelectMode
	DummySelectBtn.BackgroundColor3 = dummySelectMode and Color3.fromRGB(150, 100, 200) or Color3.fromRGB(100, 70, 150)
	DummySelectBtn.Text = dummySelectMode and "◉ Dummy Mode Active" or "👁 Click Dummy to Record"
	NPCSelectBtn.Enabled = not dummySelectMode
end)

local mouse = LocalPlayer:GetMouse()
mouse.Button1Down:Connect(function()
	if dummySelectMode then
		local target = mouse.Target
		if target then
			local humanoid = target.Parent:FindFirstChildOfClass("Humanoid")
			if humanoid then
				targetDummy = target.Parent
				DummySelectBtn.Text = "✓ Dummy: " .. targetDummy.Name
				DummySelectBtn.BackgroundColor3 = Color3.fromRGB(120, 180, 120)
				dummySelectMode = false
				updateTargetStatus()
			end
		end
	elseif npcSelectMode then
		local target = mouse.Target
		if target then
			local humanoid = target.Parent:FindFirstChildOfClass("Humanoid")
			if humanoid then
				targetNPC = target.Parent
				NPCSelectBtn.Text = "✓ NPC: " .. targetNPC.Name
				NPCSelectBtn.BackgroundColor3 = Color3.fromRGB(150, 180, 120)
				npcSelectMode = false
				updateTargetStatus()
			end
		end
...
-- NPC Selection
local npcSelectMode = false
NPCSelectBtn.MouseButton1Click:Connect(function()
	npcSelectMode = not npcSelectMode
	NPCSelectBtn.BackgroundColor3 = npcSelectMode and Color3.fromRGB(150, 180, 120) or Color3.fromRGB(100, 150, 70)
	NPCSelectBtn.Text = npcSelectMode and "◉ NPC Mode Active" or "👁 Click NPC to Play On"
	DummySelectBtn.Enabled = not npcSelectMode
end)

local function updateTargetStatus()
	local dummyName = targetDummy and targetDummy.Name or "None"
	local npcName = targetNPC and targetNPC.Name or "None"
	TargetStatusLabel.Text = "Record: " .. dummyName .. " | Play: " .. npcName
end

local RecordBtn = setupButton("⏺ Start Recording Dummy", 1, Color3.fromRGB(180, 50, 50))
...
-- HIGH ACCURACY DUMMY RECORDER BLOCK
RecordBtn.MouseButton1Click:Connect(function()
	local root = targetDummy and targetDummy:FindFirstChild("HumanoidRootPart")
	if not targetDummy or not root then
		RecordBtn.Text = "⚠ No Dummy Selected!"
		task.wait(2)
		RecordBtn.Text = "⏺ Start Recording Dummy"
		return
	end
	...
	if not isRecording then
		...
		local joints = {}
		for _, v in ipairs(targetDummy:GetDescendants()) do
...
-- INTERPOLATED SMOOTH PLAYBACK BLOCK FOR NPC
local PlayBtn = setupButton("▶ Play on NPC", 6, Color3.fromRGB(50, 120, 50))

PlayBtn.MouseButton1Click:Connect(function()
	local root = targetNPC and targetNPC:FindFirstChild("HumanoidRootPart")
	if not targetNPC or not root or isPlaying or not savedData or next(savedData) == nil then
		PlayBtn.Text = "⚠ No NPC Selected!"
		task.wait(2)
		PlayBtn.Text = "▶ Play on NPC"
		return
	end
...
local StopBtn = setupButton("⏹ Stop", 7, Color3.fromRGB(150, 85, 35))
StopBtn.MouseButton1Click:Connect(function()
	if isPlaying then forceStopPlayback = true end
end)
...
print("Delta Animation Studio (NPC Edition) successfully loaded onto screen!")
updateTargetStatus()
